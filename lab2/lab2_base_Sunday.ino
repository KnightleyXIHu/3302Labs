...
int current_state = CONTROLLER_DISTANCE_MEASURE; 
...
float t_duration = 0;
bool fflag = 0; //robot has moved forward in the loop (1) or not (0)
...
void measure_30cm_speed() {
  ...
  speedx = delta_x * delta_t; 
  delta_y = cm_ms * sin(pose_theta) * 100;
  speedy = delta_y * delta_t; 
  velocity = speedx + speedy + speedtheta; 
  sparki.print("Velocity: ");
...
//  if(fflag)
//  {
//    t_duration += delta_t; //Adding the loop time to the total duration
//  }
//  fflag = 0; //resetting flag for next loop
  displayOdometry();
